...
void Dragon::CreateNewDestination() {
  ...
  this->setSpeed(20);
  toDestination =
      new HungerDecorator(new BeelineStrategy(position, destination));
}
...
void Dragon::GetNearestEntity(std::vector<IEntity*> scheduler) {
  ...
  if (tempnearestEntity) {
    ...
    toDestination = new HungerDecorator(new BeelineStrategy(
        position, destination));  // set dragon to beeline to its new entity
  }
}
...
void Dragon::Update(double dt, std::vector<IEntity*> scheduler) {
  if (available) {  // Dragon roaming
    ...
    if (toDestination) {
      if (this->position.Distance(nearestEntity->GetPosition()) <
          20) {                             // if dragon is close enough
        if (nearestEntity->isPickedUp()) {  // if nearest entity pickedUp ==
                                            // true
          if (nearestEntity->getterNearestEntity()) {
            if (nearestEntity->getterNearestEntity()->getType() ==
                "robot") {  // checking for type
              nearestEntity->getterNearestEntity()->setDead(
                  true);  // set robot to dead to be deleted later
            }
          }
        }
        nearestEntity->SetPosition(
            {Random(-1400, 1500), position.y,
             Random(-800, 800)});  // move drone to new random location
        publisher->setMessage(std::string(details["name"]) + ": Killed " +
        ...
        setHungerLevel(getHungerLevel() +
                       dt);  // update hunger level based on time spent roaming
        destination = nearestEntity->GetPosition();
        delete toDestination;
        toDestination =
            new HungerDecorator(new BeelineStrategy(position, destination));
